        ...
        {['Alle', ...projectCategories].map((category) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-medium transition ${
              filter === category
                ? 'bg-ink text-gypsum'
                : 'border border-ink/15 bg-white text-ink-muted hover:border-ink/35 hover:text-ink'
            }`}
          >
            {category}
          </button>
      ...
      <div className="mt-10 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
        {visible.map((project, index) => (
          <Link key={project.slug} href={`/referenzen/${project.slug}`} className="group">
            ...
            <h2 className="heading-md mt-2 text-xl group-hover:text-accent">{project.title}</h2>
